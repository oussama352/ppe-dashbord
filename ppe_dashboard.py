...
df_f = pd.DataFrame({"Nom": prenoms_filles, "Genre": "Bnat (Filles)", "Pré-test": pre_f, "Post-test": post_f})
df_g = pd.DataFrame({"Nom": prenoms_garcons, "Genre": "Drari (Garçons)", "Pré-test": pre_g, "Post-test": post_g})
...
COLOR_MAP = {"Bnat (Filles)": "#E04E39", "Drari (Garçons)": "#1B6CA8"}
...
with st.sidebar:
    ...
    filtre_genre = st.selectbox("Genre", ["Tous", "Bnat (Filles)", "Drari (Garçons)"])
...
with k1:
    st.markdown(f"""<div class="kpi-card"><div class="kpi-label">👥 Effectif total</div><div class="kpi-value">{len(df_filtered)}</div><div class="kpi-delta">sur 32 élèves</div></div>""", unsafe_allow_html=True)
with k2:
    moy_pre = df_filtered["Pré-test"].mean()
    st.markdown(f"""<div class="kpi-card orange"><div class="kpi-label">📉 Moy. Pré-test</div><div class="kpi-value">{moy_pre:.1f}/20</div><div class="kpi-delta delta-down">Avant remédiation</div></div>""", unsafe_allow_html=True)
with k3:
    moy_post = df_filtered["Post-test"].mean()
    st.markdown(f"""<div class="kpi-card green"><div class="kpi-label">📈 Moy. Post-test</div><div class="kpi-value">{moy_post:.1f}/20</div><div class="kpi-delta delta-up">Après remédiation</div></div>""", unsafe_allow_html=True)
with k4:
    prog_moy = df_filtered["Progression"].mean()
    st.markdown(f"""<div class="kpi-card"><div class="kpi-label">🚀 Progression moy.</div><div class="kpi-value">+{prog_moy:.1f} pts</div><div class="kpi-delta delta-up">↑ amélioration nette</div></div>""", unsafe_allow_html=True)
with k5:
    n_maitrise = len(df_filtered[df_filtered["Niveau"] == "Maîtrise"])
    pct = int(n_maitrise / len(df_filtered) * 100) if len(df_filtered) > 0 else 0
    st.markdown(f"""<div class="kpi-card purple"><div class="kpi-label">🏆 Taux de maîtrise</div><div class="kpi-value">{pct}%</div><div class="kpi-delta delta-up">{n_maitrise} élèves</div></div>""", unsafe_allow_html=True)
 
st.markdown("")
 
tabs = st.tabs(["📊 Vue Globale","⚖️ Bnat vs Drari","🎯 Compétences","👥 Suivi Élèves","📈 Simulation","📋 Rapport"])
...
with tabs[0]:
    ...
    st.markdown("""<div class="insight-box"><b>🔍 Observation clé :</b> Tous les élèves se situent <b>au-dessus de la ligne d'égalité</b>, confirmant une progression universelle après la séance de remédiation. La majorité a dépassé le <b>seuil de maîtrise (14/20)</b>. Les stratégies de <b>guidage progressif</b> et de <b>correction collective</b> ont eu un impact significatif, notamment sur la lecture des axes (+59 pts de taux de maîtrise collectif).</div>""", unsafe_allow_html=True)
...
with tabs[1]:
    ...
    with c1:
        ...
        fig = px.bar(means.melt(id_vars="Genre", var_name="Test", value_name="Score"), x="Test", y="Score", color="Genre", barmode="group", color_discrete_map=COLOR_MAP, title="Score moyen /20 — Pré vs Post test")
    ...
    with c2:
        ...
        fig2 = px.bar(prog_genre, x="Genre", y="Prog %", color="Genre", color_discrete_map=COLOR_MAP, title="Taux de progression moyen (%)")
        fig2.update_layout(height=300, plot_bgcolor="white", paper_bgcolor="white", margin=dict(t=40,b=10,l=0,r=0), showlegend=False)
        st.plotly_chart(fig2, use_container_width=True)
 
    fig3 = px.box(df, x="Genre", y="Post-test", color="Genre", points="all", hover_name="Nom", color_discrete_map=COLOR_MAP, title="Distribution des scores post-test par genre")
    ...
    fig4 = go.Figure()
    fig4.add_trace(go.Scatterpolar(r=post_comp_f+[post_comp_f[0]], theta=comps_short+[comps_short[0]], fill='toself', name='Bnat', line_color='#E04E39', fillcolor='rgba(224,78,57,0.15)'))
    fig4.add_trace(go.Scatterpolar(r=post_comp_g+[post_comp_g[0]], theta=comps_short+[comps_short[0]], fill='toself', name='Drari', line_color='#1B6CA8', fillcolor='rgba(27,108,168,0.15)'))
...
with tabs[2]:
    ...
    for i, (comp, pre, post) in enumerate(zip(competences, pre_comp, post_comp)):
        with cols[i]:
            delta = post - pre
            st.metric(label=f"{'🔵' if i==0 else '🟢' if i==1 else '🟠' if i==2 else '🟣'} {comp}", value=f"{post}%", delta=f"+{delta}pts depuis pré-test")
    ...
    with c2:
        ...
        fig2.update_layout(title="Gain de maîtrise par compétence (%)", height=300, plot_bgcolor="white", paper_bgcolor="white", margin=dict(t=40,b=10,l=0,r=0), showlegend=False)
        st.plotly_chart(fig2, use_container_width=True)
    st.markdown('<div class="section-header">Carte thermique — maîtrise par compétence et genre</div>', unsafe_allow_html=True)
    heatmap_data = pd.DataFrame({"Bnat": post_comp_f, "Drari": post_comp_g}, index=competences)
    fig3 = px.imshow(heatmap_data, text_auto=True, aspect="auto", color_continuous_scale="Blues", labels=dict(color="Taux %"), title="Taux de maîtrise post-test (%)")
...
with tabs[4]:
    ...
    with c2:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=seances, y=score_simul, mode="lines+markers", line=dict(color="#1B6CA8", width=3), marker=dict(size=10, color="#1B6CA8"), fill="tozeroy", fillcolor="rgba(27,108,168,0.1)", name="Score simulé"))
    ...
    recs = []
    if intensite_guidage < 50: recs.append("⚠️ Augmentez l'intensité du guidage progressif pour mieux soutenir les élèves en difficulté.")
    if travail_groupe < 50: recs.append("⚠️ Le travail en groupes favorise l'entraide et la co-construction du savoir — à renforcer.")
    if correction_coll < 60: recs.append("⚠️ La correction collective renforce la métacognition et la révision des erreurs.")
    if nb_seances < 2: recs.append("💡 Une 2ème séance de soutien permettrait de consolider les acquis, surtout pour les conclusions.")
    if not recs: recs.append("✅ Excellent dispositif ! Vos stratégies couvrent toutes les dimensions de la remédiation efficace.")
    for r in recs:
        st.markdown(f"- {r}")
...
Il porte sur la **remédiation des difficultés** rencontrées par les élèves de 3AC dans l'interprétation des graphes en immunologie.
...
|---|---|---|---|
| Bnat (Filles) | {df_f['Pré-test'].mean():.1f}/20 | {df_f['Post-test'].mean():.1f}/20 | +{df_f['Post-test'].mean()-df_f['Pré-test'].mean():.1f} pts |
| Drari (Garçons) | {df_g['Pré-test'].mean():.1f}/20 | {df_g['Post-test'].mean():.1f}/20 | +{df_g['Post-test'].mean()-df_g['Pré-test'].mean():.1f} pts |
